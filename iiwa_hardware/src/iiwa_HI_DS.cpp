...
// ------------------------------------------------------------------------------------------
CallbackReturn IiwaDirectServoPositionHardwareInterface::on_activate(
  const rclcpp_lifecycle::State & previous_state)
{
  RCLCPP_INFO(
    rclcpp::get_logger(
      "IiwaDirectServoPositionHardwareInterface"), "Starting ...please wait...");
  ...
  RCLCPP_INFO(
    rclcpp::get_logger(
      "IiwaDirectServoPositionHardwareInterface"), "Connecting to port= %i and ip= %s", p_port,
    p_ip);
  ...
  while (!socketConnected_) {
    socket_status_ = sendto(
      destSocket_, msg_data_send.c_str(),
      msg_data_send.length(), NO_FLAGS_SET, reinterpret_cast<const sockaddr *>( &destSockAddr_to_),
      sizeof(destSockAddr_to_));
    if (socket_status_ != msg_data_send.length()) {
      RCLCPP_FATAL(
        rclcpp::get_logger(
          "IiwaDirectServoPositionHardwareInterface"), "ERROR: sendto unsuccessful in first send.");
      return CallbackReturn::ERROR;
    }
    socket_status_ =
      recvfrom(
      destSocket_, buffer_, MAXBUFLEN, NO_FLAGS_SET,
      reinterpret_cast<sockaddr *>(&destSockAddr_from_), &fromlen_);
    if (socket_status_ == SOCKET_ERROR) {
      RCLCPP_FATAL(
        rclcpp::get_logger(
          "IiwaDirectServoPositionHardwareInterface"),
      ...
      if (state_elements[0] == "iiwa_joint_state" &&
        state_elements.size() == hw_states_.size() + 1)
      {
        for (int i = 1; i < state_elements.size(); i++) {
          hw_states_[i - 1] = atof(state_elements[i].c_str());
        }
      }
  ...
  std::cout << "Initial joint position: " << hw_states_[0] << ", " << hw_states_[1] << ", " <<
    hw_states_[2] << ", " << hw_states_[3] << ", " << hw_states_[4] << ", " << hw_states_[5] <<
    ", " << hw_states_[6] << std::endl;
...
CallbackReturn IiwaDirectServoPositionHardwareInterface::on_deactivate(
  const rclcpp_lifecycle::State & previous_state)
{
  RCLCPP_INFO(
    rclcpp::get_logger(
      "IiwaDirectServoPositionHardwareInterface"), "Stopping ...please wait...");
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  if (socket_status_ == SOCKET_ERROR) {
    RCLCPP_FATAL(
      rclcpp::get_logger(
        "IiwaDirectServoPositionHardwareInterface"), "ERROR: recvfrom unsuccessful.");
